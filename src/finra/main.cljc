(ns finra.main
  "Kotodama WASM entrypoint for the Finra clean-room actor (L4) — Clojure port.
  ...
  facts (`finra.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"finra.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "finra-compat" :tier tier :entities entities} 200])
